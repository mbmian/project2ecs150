...
int uthread_run(bool preempt, uthread_func_t func, void *arg)
{
    ...
    idle_thread = malloc(sizeof(uthread_tcb_type));
    if (idle_thread == NULL) {
        return -1;
    }
    ...
    return (idle_thread->stack == NULL) ? (free(idle_thread), -1) : 0;


    // Initialize the idle thread's context but leave its function as NULL
    return (uthread_ctx_init(&(idle_thread->context), idle_thread->stack, NULL, NULL) == -1) ?
    (uthread_ctx_destroy_stack(idle_thread->stack), free(idle_thread), -1) : 0;
